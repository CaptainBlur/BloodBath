...
import com.vova9110.bloodbath.alarmsUI.MainDrawables;
...
//        prefFrame.setAnimation();
    ...
    private void setupRepeatContainer(Alarm alarm){
        ...
        final boolean shiftedWeek = view.getContext().getSharedPreferences(MainViewModel.USER_SETTINGS_NAME, Context.MODE_PRIVATE)
...
//        hourPicker.setVisibility(View.VISIBLE); minutePicker.setVisibility(View.VISIBLE); switcher.setVisibility(View.VISIBLE); FAB.setVisibility(View.VISIBLE);
        ...
        prefRepeat.setChecked(current.getRepeatable());
//        savedWeekdays = new boolean[]{true, false, true, false, true, false, true};
        ...
        prefActiveness.setChecked(current.getDetection());
        ...
        prefActiveness.setOnClickListener(v ->{
            ...
            if (view.isChecked()) view.setForeground(drawables.get_checked_drawable(MainDrawables.rv_pref_activeness));
            else view.setForeground(drawables.get_unchecked_drawable(MainDrawables.rv_pref_activeness));
...
//            sl.sp( "onClick: ERROR");
    ...
    public void startTimeWindowAlphaAnimation(long duration, float toValue){
        ...
        ViewPropertyAnimator animator =  timeWindow.animate();

        animator.setDuration(duration).alpha(toValue).setInterpolator(interpolator).start();
    }
    ...
    public void hidePrefNLaunchHandler(long duration){
        ...
        animator.setDuration(duration).alpha(0f).setInterpolator(interpolator).setListener(listener).start();
    }
